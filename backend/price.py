...
- Растаможка РФ: таможенный сбор, пошлина, акциз, утилизационный сбор, НДС 22%%.
- Брокер BROKER_RUB и комиссия (10%% от базы или ≈400 000 ₽ для дорогих авто).

Правила пошлины / утиля / НДС ориентированы на режим физлиц ЕАЭС 2026 и официальные
ставки (единые ставки ЕТС €/см³, шкала для авто младше 3 лет, НДС с базы ввоза).
...
VAT_RATE = 0.22  # Базовая ставка НДС 2026

# НК РФ ст. 193: акциз по мощности двигателя (руб./л.с.), ориентир на 2026.
# Ставки храним в конфиге, здесь — значения по умолчанию.
# Формат: (верхняя_граница_лс, ставка_руб_за_лс)
...
def excise_rub(power_hp: Optional[float], hp_tiers: Optional[List[Tuple[float, float]]] = None) -> float:
    """
    Акциз: ставка (руб./л.с.) * мощность.
    Для гибридов и EV используем доступную мощность из данных авто.
    """
    if power_hp is None or power_hp <= 0:
        return 0.0
    tiers = hp_tiers or EXCISE_HP_TIERS_RUB_PER_HP
    hp = float(power_hp)
    rate = tiers[-1][1]
    for hp_limit, r in tiers:
        if hp <= hp_limit:
            rate = r
            break
    return hp * rate
...
def vat_import_rub(
    ...
    *,
    ...
    """
    НДС для физлица (личное пользование), 2026:
    - EV до 5 лет включительно: 0%
    - EV старше 5 лет: 22% от (стоимость + пошлина + акциз)
    - ICE / hybrid: 0%
    """
    if fuel != "electric":
        return 0.0
    if age_years <= 5:
        return 0.0
    base = max(0.0, car_value_rub + duty_rub + excise_value_rub)
