...
# ``price`` в 만원 или ``price_won``/1e4; ниже ~4.8M ₩ для относительно свежих авто чаще «приманка» лизинга, не наличная.
...
def classify_encar_price_intent(
    ...
    *,
    ...
    if (
        0 < eff_mw < _ENCAR_SUSPICIOUS_MAX_EFF_MW
        and ((year_val is not None and year_val >= 2015) or (km_val is not None and km_val <= 150000))
    ):
