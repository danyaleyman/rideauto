#!/usr/bin/env python3
"""Помечает строки Che168 на пересчёт цены: needs_pricing_recompute (china_json_suggests_pricing_resync)."""
...
def _dsn_for_host_outside_compose(dsn: str) -> str:
    ...
    print(
        "repair_china_pricing_recompute_queue: DSN host postgres → 127.0.0.1",
        file=sys.stderr,
    )
    return out


def _fetch_che168_candidates(
    dsn: str,
    *,
    limit: int,
...
def main() -> int:
    from pricechina import china_json_suggests_pricing_resync
    ...
    rows = _fetch_che168_candidates(dsn, limit=max(1, args.limit), car_ids=args.car_id or None)
    to_mark: List[int] = []
    for rid, _cid, data in rows:
        if china_json_suggests_pricing_resync(data):
            to_mark.append(rid)
