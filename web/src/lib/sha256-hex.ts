/** SHA-256 hex digest; sync fallback when Web Crypto subtle недоступен (HTTP не на localhost). */

function sha256HexUtf8Sync(message: string): string {
  const K = [
  ...
  for (let i = 0; i < withOne.length; i += 64) {
    const dv = new DataView(withOne.buffer, withOne.byteOffset + i, 64);
