...
2. Синхронизация каталога / экспорт: если `power` пусто → `get_power_for_car()` → **engine_map** → `power_lookup.json` (см. `postgres_catalog_sync`, `catalog_export_utils`).
...
1. переменная **`DATABASE_URL`** (обход строк `cars` в Postgres), иначе  
2. `web/public/cars.json`, иначе чанки `web/public/data/chunks/cars_*.json`, иначе  
3. файлы в `data/` (если положите туда дамп)
...
После синхронизации каталога (опционально):
...
# после encar_scraper (prod): в /etc/default/rideauto задайте WRA_LEARN_ENGINE_MAP=1 — иначе шаг не вызывается.
```
